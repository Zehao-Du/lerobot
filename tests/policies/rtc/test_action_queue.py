...
def test_peek_requires_hardware_acknowledgement(action_queue_rtc_enabled, sample_actions):
    action_queue_rtc_enabled.merge(sample_actions["original"], sample_actions["processed"], real_delay=0)

    first = action_queue_rtc_enabled.peek()
    assert torch.equal(first, sample_actions["processed"][0])
    assert action_queue_rtc_enabled.last_index == 0
    assert action_queue_rtc_enabled.get_progress_snapshot()[0] == 0

    assert action_queue_rtc_enabled.acknowledge()
    assert action_queue_rtc_enabled.last_index == 1
    assert action_queue_rtc_enabled.get_progress_snapshot()[0] == 1
    assert torch.equal(action_queue_rtc_enabled.peek(), sample_actions["processed"][1])


def test_merge_maps_inflight_action_to_new_chunk_front(action_queue_rtc_enabled, sample_actions):
    action_queue_rtc_enabled.merge(sample_actions["original"], sample_actions["processed"], real_delay=0)
    assert torch.equal(action_queue_rtc_enabled.peek(), sample_actions["processed"][0])
    completed_before, epoch = action_queue_rtc_enabled.get_progress_snapshot()

    new_original = sample_actions["original"] + 100
    new_processed = sample_actions["processed"] + 100
    assert action_queue_rtc_enabled.merge(
        new_original,
        new_processed,
        real_delay=0,
        expected_epoch=epoch,
        completed_count_before_inference=completed_before,
    )

    # The new front represents the RTC-guided equivalent of the physical in-flight
    # action. Its eventual ACK advances to the next action in the new chunk.
    assert torch.equal(action_queue_rtc_enabled.peek(), new_processed[0])
    assert action_queue_rtc_enabled.acknowledge()
    assert torch.equal(action_queue_rtc_enabled.peek(), new_processed[1])


def test_merge_discards_result_from_previous_queue_epoch(action_queue_rtc_enabled, sample_actions):
    _, epoch = action_queue_rtc_enabled.get_progress_snapshot()
    action_queue_rtc_enabled.clear()

    assert not action_queue_rtc_enabled.merge(
...
def test_merge_resolves_completed_delay_atomically(action_queue_rtc_enabled, sample_actions):
    action_queue_rtc_enabled.merge(sample_actions["original"], sample_actions["processed"], real_delay=0)
    completed_before, epoch = action_queue_rtc_enabled.get_progress_snapshot()
    assert action_queue_rtc_enabled.acknowledge()
