...
## Inspect a Full Action Chunk Without Moving the Robot

Use the one-shot `action_debug` strategy to capture the robot's current observation,
run `predict_action_chunk()` once, and print every predicted action in the terminal:

```bash
lerobot-rollout \
    --strategy.type=action_debug \
...
The output contains the raw model tensor and postprocessed values grouped as
position, rotation, and gripper channels for every horizon step. This strategy never
calls `robot.send_action()` and exits after the single prediction.
